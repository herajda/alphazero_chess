...
class Agent:
    ...
    def load(cls, path: str, args=None) -> "Agent":
        ...
        if isinstance(checkpoint, dict) and "state_dict" in checkpoint:
            saved_args_dict = checkpoint.get("model_args") or {}
            if saved_args_dict:
                load_args = argparse.Namespace(**saved_args_dict)
                if args is not None:
                    runtime_overrides = {
                        "compile",
                        "compile_backend",
                    ...
                    for key, value in vars(args).items():
                        if key not in saved_args_dict or key in runtime_overrides:
                            setattr(load_args, key, value)
            elif args is not None:
                load_args = args
            else:
                raise ValueError("args must be provided when loading checkpoints without model_args")
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"('{getattr(load_args, 'model_arch', 'transformer')}')."
            ) from exc
        agent._model = agent._model.to(agent.device)
        return agent

    @staticmethod
    def _checkpoint_arg_value(value) -> tuple[bool, object]:
        if value is None or isinstance(value, (str, int, float, bool)):
            return True, value
        if isinstance(value, os.PathLike):
            return True, os.fspath(value)
        if isinstance(value, (list, tuple)):
            converted = []
            for item in value:
                keep, item_value = Agent._checkpoint_arg_value(item)
                if keep:
                    converted.append(item_value)
            return True, converted
        return False, None

    def _checkpoint_args(self) -> dict[str, object]:
        ...
        for key, value in vars(self.args).items():
            if key.startswith("_") or callable(value):
                continue
            keep, checkpoint_value = self._checkpoint_arg_value(value)
            if keep:
    ...
    def save(self, path: str) -> None:
        ...
        torch.save({"state_dict": state, "model_args": self._checkpoint_args()}, path)
